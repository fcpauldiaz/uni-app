...
import type {Career} from "../components/food/Model";
...
export default class CategoryComp extends React.Component<NavigationProps<{ categoryId: string }>> {
    ...
    renderItem = (career: Career): React.Node => {
        ...
        const description = `${career.years} ${career.years > 1 ? "años" : "año"} · ${career.credits} créditos`;
        return (
            <Card
                height={250}
                onPress={() => navigation.navigate("Career", { categoryId, recipeId: career.id })}
                {...{description}}
                {...career}
            />
        );
